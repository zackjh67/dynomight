...
func _ready():
	...
	for character in characters:
		for stop in char_stops[character]:
			stop.connect('body_entered', Callable(self, 'stop').bind(stop, character))
...
#		dialogues[character].connect('dialogue_closed', self, 'dialogue_closed', [character])
	...
	await dialogues['chad'].dialogue_closed
...
func _physics_process(delta):
	...
	for character in characters:
		...
		var path_follow:PathFollow2D = path_follows[character]
		var anim_tree = anim_trees[character]
		
		if !stopped[character]:
			path_follow.set_progress(path_follow.get_progress() + char_speed * delta)
...
func start_game_store_interaction():
	...
	await get_tree().create_timer(0.5/G.FAST_FORWARD).timeout
...
func start_library_interaction():
	...
	await get_tree().create_timer(4/G.FAST_FORWARD).timeout
	...
	await get_tree().create_timer(2/G.FAST_FORWARD).timeout
	start_character('jennay', 75*G.FAST_FORWARD)
	await char_stops['jennay'][3].body_entered
...
func start_dirt_pit_interaction():
	...
	await dialogues['jennay'].message_finished
...
func start_farewell_interaction():
	...
	await dialogues['player'].message_finished
	...
	await dialogues['player'].dialogue_closed
	await get_tree().create_timer(1/G.FAST_FORWARD).timeout
